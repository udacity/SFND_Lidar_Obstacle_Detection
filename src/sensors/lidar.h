...
struct Ray
{
	...
	void rayCast(const std::vector<Car>& cars, double minDistance, double maxDistance, pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud, double slopeAngle, double sderr)
	{
		...
		while(!collision && castDistance < maxDistance)
		{
			...
			// FIX:: you should subtract origin.x if ray origin is not at (0, 0, 0)
...
struct Lidar
{
		...
		: cloud(new pcl::PointCloud<pcl::PointXYZ>()), position(0,0,2.6)
	{
		...
		maxDistance = 50;
		resolution = 0.2;
		// set sderr to higher values to get more realistic pcd files
		sderr = 0.2;
		cars = setCars;
		...
		// set to smaller values to get higher resoultion pcd
